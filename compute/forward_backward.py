...
class _QConv2dFunc(torch.autograd.Function):
    ...
    def forward(
        ...
        if bias is not None:
            bias = bias.round().clamp(-(2**31) + 1, 2**31 - 1)
        else:
            bias = None
    ...
    def backward(ctx, grad_output):
        ...
        q_grad_output = (
            quantize_tensor(_grad_output, scale, zero_point, num_bits=gbit, signed=True)
            - zero_point
        )
...
class _QLinearFunc(torch.autograd.Function):
    ...
    @staticmethod
    def backward(ctx, grad_output):
        weight, M, x, out, wzero, izero, ozero, wscale, iscale, oscale = (
            ctx.saved_tensors
        )
        ...
        grad_x = dequantize_tensor(grad_x, scale=scale, zero_point=0.0)
...
class _QAddFunct(torch.autograd.Function):
    ...
    def forward(ctx, x1, x2, qi1, qi2, qo):
        ...
        ctx.M1 = M1
        ctx.M2 = M2
        x = (x1 - qi1.zero_point) * M1 + (x2 - qi2.zero_point) * M2
        x.round_()
        x = x + qo.zero_point
        return x
...
class _QAvgpoolFunc(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = (
            grad_output.repeat(1, 1, *input_shape[-2:])
