...
import torch
from models.fpn import Resnet18_FPN
from models.quantize_class import QAddReLU, QConvBNReLU, QMaxPooling2d
...
class QuantizeResidual(nn.Module):
    ...
    def quantize(self, training=True):
        self.qlayer1 = QConvBNReLU(
            self.residual.layer1.conv,
            bn_module=self.residual.layer1.bn,
            qi=False,
            qo=True,
            w_bits=self.w_bits,
            a_bits=self.a_bits,
            activate=self.residual.layer1.activate,
            signed=self.signed,
            bl=self.bl,
        )
        self.qlayer2 = QConvBNReLU(
            self.residual.layer2.conv,
            bn_module=self.residual.layer2.bn,
            qi=False,
            qo=True,
            w_bits=self.w_bits,
            a_bits=self.a_bits,
            activate=self.residual.layer2.activate,
            signed=self.signed,
            bl=self.bl,
        )
        self.qlayer3 = QConvBNReLU(
            self.residual.layer3.conv,
            bn_module=self.residual.layer3.bn,
            qi=False,
            qo=True,
            w_bits=self.w_bits,
            a_bits=self.a_bits,
            activate=self.residual.layer3.activate,
            signed=self.signed,
            bl=self.bl,
        )
        self.qadd = QAddReLU(
            ...
            a_bits=self.a_bits,
            signed=self.signed,
            activate=hasattr(self.residual, "relu"),
        )

        if self.ineqout is not True:
            self.qshortcut = QConvBNReLU(
                self.residual.shortcut.conv,
                bn_module=self.residual.shortcut.bn,
                qi=False,
                qo=True,
                w_bits=self.w_bits,
                a_bits=self.a_bits,
                activate=self.residual.shortcut.activate,
                signed=self.signed,
                bl=self.bl,
            )
...
class QuantizeResnet18(Resnet18_FPN):
    def __init__(self, num_class, signed=True, w_bits=8, a_bits=8):
        super(QuantizeResnet18, self).__init__(
            num_classes=num_class, img_size=448, strides=[8, 16, 32]
        )
        self.signed = signed
        self.w_bits = w_bits
        self.a_bits = a_bits
    ...
    def quantize(self):
        ...
        self.qneck5 = QuantizeResidual(
            self.neck5, w_bits=self.w_bits, a_bits=self.a_bits, signed=self.signed, bl=0
        )
        self.qneck5.quantize()

        self.qneck4 = QuantizeResidual(
            self.neck4, w_bits=self.w_bits, a_bits=self.a_bits, signed=self.signed, bl=0
        )
        self.qneck4.quantize()

        self.qneck3 = QuantizeResidual(
            self.neck3, w_bits=self.w_bits, a_bits=self.a_bits, signed=self.signed, bl=0
        )
        ...
        for i in range(len(self.net.feature)):
            self.qfeature[-1 - i] = QuantizeInvertedResidual_resnet18(
                self.net.feature[-1 - i],
                w_bits=self.w_bits,
                a_bits=self.a_bits,
                signed=self.signed,
                bl=bl,
        ...
        if not isinstance(self.net.maxpool, nn.Identity):
            self.qmaxpool = QMaxPooling2d(
                a_bits=self.a_bits,
                kernel_size=self.net.maxpool.kernel_size,
                stride=self.net.maxpool.stride,
                padding=self.net.maxpool.padding,
                qi=False,
                qo=True,
                signed=self.signed,
            )

        self.qlayer0 = QConvBNReLU(
            self.net.layer0.conv,
            self.net.layer0.bn,
            qi=True,
            qo=True,
            w_bits=self.w_bits,
    ...
    def freeze(self):
        ...
        self.qadd4.freeze(
            self.qneck4.get_last_layer_qo(), self.qneck5.get_last_layer_qo()
        )
