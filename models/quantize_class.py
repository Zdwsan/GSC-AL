...
from compute.forward_backward import (
    _QAddFunct,
    _QAvgpoolFunc,
    _QConv2dFunc,
    _QLinearFunc,
...
class QLinear(QModule):
    ...
    def freeze(self, qi=None, qo=None):
        ...
        self.fc_module.bias.data = torch.nn.Parameter(
            quantize_tensor(
    ...
    def update_quantize_parameters(self):
        self.M, self.fc_module = self.qw.update_quantize_parameters(
            self.fc_module, self.M, self.qi, self.qo
        )
...
class QConvBNReLU(QModule):
        ...
        if self.bn_module is not None:
            if self.bn_module is not None:
                if self.training:
                    ...
                    y = y.contiguous().view(
                        self.conv_module.out_channels, -1
                    )  # CNHW -> C,NHW
                    mean = y.mean(1)
                    var = y.var(1)
                    self.bn_module.running_mean = (
                        1 - self.bn_module.momentum
                    ) * self.bn_module.running_mean + self.bn_module.momentum * mean  # .detach()
                    self.bn_module.running_var = (
                        1 - self.bn_module.momentum
                    ) * self.bn_module.running_var + self.bn_module.momentum * var  # .detach()
                else:
    ...
    def freeze(self, qi=None, qo=None):
        ...
        self.M.data = (self.qw.scale * self.qi.scale / self.qo.scale).data.view(
            1, -1, 1, 1
        )
        ...
        if self.bn_module is not None:
            ...
            weight, bias = self.fold_bn(
                self.conv_module, self.bn_module.running_mean, std
            )
        else:
            weight, bias = self.conv_module.weight, self.conv_module.bias
        ...
        self.conv_module.bias = torch.nn.Parameter(
            quantize_tensor(
                bias,
    ...
    def update_quantize_parameters(self):
        self.M, self.conv_module = self.qw.update_quantize_parameters(
            self.conv_module, self.M, self.qi, self.qo
        )
    ...
    def quantize_inference(self, x):
        out = self.QConv2d(
            ...
            self.conv_module.weight,
            self.conv_module.bias,
            self.conv_module.stride,
            self.conv_module.padding,
            self.conv_module.dilation,
...
class QAddReLU(QModule):
    def __init__(
        self, a_bits, qi1=None, qi2=None, qo=None, signed=True, activate=False
    ):
...
class QAvgPooling2d(QModule):
    def __init__(
        self, a_bits, kernel_size=7, stride=1, padding=0, qi=False, qo=True, signed=True
    ):
