    ...
    def pre_step(model, e, epoch, tlayer):
        for n, m in model.named_children():
            if isinstance(m, QConvBNReLU):
                if (
                    m.conv_module.bias.grad is not None
                    and m.conv_module.kernel_size[0] > 0
                ):
                    tscale = (m.qw.scale * m.qi.scale) ** 2
                    m.conv_module.bias.grad.data = m.conv_module.bias.grad.data / tscale

                if (
                    m.conv_module.weight.grad is not None
                    and m.conv_module.kernel_size[0] > 0
                ):
                    c_max = (
                        m.conv_module.weight.grad.abs()
                        .contiguous()
                        .view(m.conv_module.weight.grad.shape[0], -1)
                        .max(dim=1)
                        .values
                    )  # 按通道来
                    mask0 = c_max == 0
                    ...
                    m.conv_module.weight.grad.data = (
                        m.conv_module.weight.grad.data * tscale3.view(-1, 1, 1, 1)
                    )
            ...
            if isinstance(m, QLinear):
                ...
                if m.fc_module.weight.grad is not None:
                    ...
                    m.fc_module.weight.grad.data = (
                        m.fc_module.weight.grad.data * tscale3.view(-1, 1)
                    )
